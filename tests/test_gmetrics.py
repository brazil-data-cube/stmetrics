...
def test_polar():
    ...
    polares = stmetrics.utils.error_polar()

    if all(numpy.isnan(value) != numpy.nan for value in polares.values()):
        pass


# def test_fractal():

#     fractal = stmetrics.utils.error_fractal()

#     if all(numpy.isnan(value) != numpy.nan for value in fractal.values()):
#         pass


# def test_basics():

#     basics = stmetrics.utils.error_basics()

#     if all(numpy.isnan(value) != numpy.nan for value in basics.values()):
#         pass
...
def test_symmetric_distance_ii():
    ...
    dist = stmetrics.polar.symmetric_distance(s1, s2)
...
def test_geometries():
    ...
    res = [stmetrics.spatial.symmetry(poly),
           stmetrics.spatial.aspect_ratio(poly),
           stmetrics.spatial.reock_compactness(poly),
           stmetrics.spatial.rectangular_fit(poly),
           stmetrics.spatial.width(poly),
           stmetrics.spatial.length(poly)]
